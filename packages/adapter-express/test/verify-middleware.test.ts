...
function buildApp(opts: Parameters<typeof verifyMiddleware>[1] = { secret }) {
	const app = express();
	app.post("/webhooks/stripe", verifyMiddleware("stripe", opts), (req, res) => {
		const verified = req as unknown as VerifiedIncomingMessage;
		res.status(200).json({ received: true, event: verified.hookkitEvent });
	});
	return app;
}

describe("adapter-express verifyMiddleware", () => {
	it("accepts a validly signed event and exposes rawBody + hookkitEvent to the handler", async () => {
	...
	it("rejects a tampered signature with a generic 400 (never leaking the reason)", async () => {
		const target = toTarget(buildApp(), "/webhooks/stripe");
		const evt = generate("stripe", "checkout.session.completed", { secret });
